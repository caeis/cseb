...
if ($selectedClasse) {
    ...
    $synthese['difference'] = ($synthese['total_du_classe'] ?? 0) - ($synthese['total_du_fp'] ?? 0);

    // Requête de détails pour tous les élèves de la classe
    $sqlDetails = "
        SELECT f.Id_paie, l.Id_liste, l.Nom_Prenoms, l.Sexe, l.Type,
               f.Classe, f.Regime, f.Montant_paye, f.Montan_du, f.Date_paie, f.Fp
        FROM Fiche_paie f
    ...
    $stmt2->bind_param("s", $selectedClasse);
    $stmt2->execute();
...
<body>
    ...
    <?php if ($synthese): ?>
        ...
        <table>
            ...
            <tr>
                ...
                <td><?= $synthese['difference'] ?? 0 ?></td>
            </tr>
        </table>
    <?php endif; ?>

    <?php if ($details && $details->num_rows > 0): ?>
        <h2>Liste des élèves - Classe <?= htmlspecialchars($selectedClasse) ?></h2>
        <table>
            <tr>
                <th>ID Paie</th>
                <th>ID Élève</th>
    ...
    <?php elseif ($selectedClasse): ?>
        <p style="text-align:center;color:red;">Aucun élève trouvé pour cette classe.</p>
    <?php endif; ?>
</body>
</html>
